import { Button } from "@/components/ui/button";
import Image from "next/image";

export const FistFightALion = () => {
  return (
    <div className="py-20 container mx-auto">
      <h2 className="text-6xl font-black text-(--color-para) text-center uppercase">
        The Fist Fight a Lion <br />
        <span className="text-(--color-easesmith)">Guarantee</span>
      </h2>
      <p className="max-w-lg mx-auto text-center text-(--color-para) my-5">
        We’re so confident your new site will outperform your old one, you’ll be
        ready to fist fight a lion just to keep it live.
      </p>
      <p className="text-(--color-easesmith) font-bold text-center text-lg">
        No final payment until your site beats the old one <br /> — within 30
        days.
      </p>
      <p className="text-[#D53D3D] font-bold text-center text-lg my-3">
        If it doesn’t, you don’t pay!
      </p>
      <p className="text-(--color-easesmith) font-bold text-center text-lg">
        If it flops, you get a ₹75,000 CRO audit by a <br /> certified expert —
        absolutely free
      </p>

      <div className="mt-8 flex justify-center items-center">
        <Button className="px-8 py-5.5 text-base font-semibold rounded-sm border-2 border-transparent hover:bg-(--color-para) hover:text-(--color-easesmith)">
          Increase Your Conversions Today
        </Button>
      </div>

      <Image
        src="/images/lion.png"
        alt="Lion"
        width={500}
        height={700}
        className="mx-auto mt-8"
      />

      <h3 className="text-4xl font-black leading-12 text-center mt-6">
        TURN YOUR <span className="text-(--color-easesmith)">VISITORS</span>{" "}
        INTO <br />
        <span className="text-(--color-easesmith) underline">
          PAYING CUSTOMERS
        </span>
      </h3>
      <p className="text-center text-(--color-para) mt-3 text-lg">
        We only take on
      </p>
      <p className="font-bold italic text-center text-lg text-(--color-para)">
        5 new F&B projects per month
      </p>
      <p className="text-center text-lg text-(--color-para)">
        and they fill up{" "}
        <span className="text-(--color-easesmith) font-bold">fast</span>
      </p>

      <h3 className="text-4xl font-black leading-12 uppercase text-center mt-6">
        Book a call{" "}
        <span className="text-(--color-easesmith) underline">now!</span>
      </h3>

      <p className="text-[#D53D3D] uppercase font-bold text-center text-lg">
        before the bonuses disappear
      </p>
      <p className="text-[#D53D3D] uppercase font-bold text-center text-lg my-3 mt-1">
        (We only work with F&B brands doing ₹10+L in Monthly revenue)
      </p>

      <div className="mt-8 flex justify-center items-center">
        <Button className="px-8 py-5.5 text-base font-semibold rounded-sm border-2 border-transparent hover:bg-(--color-para) hover:text-(--color-easesmith)">
          Increase Your Conversions Today
        </Button>
      </div>
    </div>
  );
};
